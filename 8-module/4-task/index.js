...
export default class Cart {
  ...
  renderProduct(product, count) {
    //Возвращает верстку товара, для показа в корзине
  ...
  onProductUpdate({product, count}) {
    ...
    if (count == 0) {
      ...
      // Элемент, который хранит количество товаров с таким productId в корзине    //вставляем новое количество единиц товара
      this.modalBody.querySelector(`[data-product-id="${product.id}"] .cart-counter__count`).innerHTML = count;
    // Элемент с общей стоимостью всех единиц этого товара                         //вставляем новую стоимость этого вида
      this.modalBody.querySelector(`[data-product-id="${product.id}"] .cart-product__price`).innerHTML = '€' + (count * product.price).toFixed(2);
    }
    // Элемент с суммарной стоимостью всех товаров                                //вставляем новую стоимость корзины
